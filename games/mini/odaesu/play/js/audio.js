/* 오대수 — 소리. 전부 WebAudio 합성(파일 없음, file:// 에서도 난다).
...
(function () {
  ...
  try { on = localStorage.getItem('odaesu.snd') !== '0'; music = localStorage.getItem('odaesu.bgm') !== '0'; } catch (e) {}
  ...
  // ── 음악: 직접 쓴 단조 왈츠(3/4, 138bpm). 톱니 두 줄을 겹쳐 현악처럼 ──
  var MEL = [
    [[76, 2], [74, 1]], [[72, 1], [71, 1], [72, 1]], [[74, 2], [77, 1]], [[76, 3]],
    [[71, 2], [72, 1]], [[74, 1], [72, 1], [71, 1]], [[69, 3]], [[64, 1], [69, 1], [71, 1]],
    [[72, 2], [74, 1]], [[76, 1], [77, 1], [76, 1]], [[74, 2], [72, 1]], [[71, 1], [72, 1], [74, 1]],
    [[71, 1], [68, 1], [71, 1]], [[74, 2], [71, 1]], [[69, 3]], [[0, 3]]
  ];
  var CH = [[45, 57, 60, 64], [45, 57, 60, 64], [50, 57, 62, 65], [50, 57, 62, 65],
            [40, 56, 59, 64], [40, 56, 59, 64], [45, 57, 60, 64], [45, 57, 60, 64],
            [41, 57, 60, 65], [41, 57, 60, 65], [50, 57, 62, 65], [50, 57, 62, 65],
            [40, 56, 59, 62], [40, 56, 59, 64], [45, 57, 60, 64], [45, 57, 60, 64]];
  var BEAT = 60 / 138, playing = false, nextBar = 0, barIdx = 0, timer = null;

  function strings(m, t, dur, vol) {
    var c = ctx, f = mtof(m);
    var g = c.createGain();
    g.gain.setValueAtTime(0.0001, t); g.gain.linearRampToValueAtTime(vol, t + 0.09);
    g.gain.setValueAtTime(vol, t + dur * 0.8); g.gain.exponentialRampToValueAtTime(0.0001, t + dur + 0.25);
    var lp = c.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 1900; lp.Q.value = 0.6;
    var vib = c.createOscillator(); vib.frequency.value = 5.2; var vg = c.createGain(); vg.gain.value = f * 0.006;
    vib.connect(vg);
    [-6, 6].forEach(function (cents) {
      var o = c.createOscillator(); o.type = 'sawtooth'; o.frequency.value = f; o.detune.value = cents;
      vg.connect(o.frequency); o.connect(lp); o.start(t); o.stop(t + dur + 0.3);
    });
    vib.start(t); vib.stop(t + dur + 0.3);
    lp.connect(g); g.connect(mus);
  }
  function pluck(m, t, vol, type) {
    var c = ctx, o = c.createOscillator(); o.type = type || 'triangle'; o.frequency.value = mtof(m);
    var lp = c.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 900;
    var g = c.createGain(); g.gain.setValueAtTime(vol, t); g.gain.exponentialRampToValueAtTime(0.0001, t + BEAT * 0.9);
    o.connect(lp); lp.connect(g); g.connect(mus); o.start(t); o.stop(t + BEAT);
  }
  function schedule() {
    if (!ctx || !playing) return;
    while (nextBar < ctx.currentTime + 0.6) {
      var b = barIdx % 16, ch = CH[b], t = nextBar;
      pluck(ch[0] - 12, t, 0.34, 'sawtooth');                     // 쿵
      pluck(ch[1], t + BEAT, 0.09); pluck(ch[2], t + BEAT, 0.09); pluck(ch[3], t + BEAT, 0.09);   // 짝
      pluck(ch[1], t + BEAT * 2, 0.08); pluck(ch[2], t + BEAT * 2, 0.08); pluck(ch[3], t + BEAT * 2, 0.08);
      var bt = t;
      MEL[b].forEach(function (n) { if (n[0]) strings(n[0], bt, n[1] * BEAT, 0.05); bt += n[1] * BEAT; });
      nextBar += BEAT * 3; barIdx++;
    }
  }
  function bgm(play) {
    var c = live(); if (!c) return;
    if (play && !playing) { playing = true; nextBar = c.currentTime + 0.1; barIdx = 0; timer = setInterval(schedule, 150); schedule(); }
  ...
  window.SND = {
    ...
    eat: eat, clear: clear, ding: ding, hurt: hurt, beat: beat, bgm: bgm,
    ...
    setOn: function (b) { on = b; try { localStorage.setItem('odaesu.snd', b ? '1' : '0'); } catch (e) {} if (sfx) sfx.gain.value = b ? 1 : 0; },
    setMusic: function (b) { music = b; try { localStorage.setItem('odaesu.bgm', b ? '1' : '0'); } catch (e) {} if (mus) mus.gain.value = b ? 0.55 : 0; }
  };
})();
